...
// --- NAVBAR LOGIC (Aggressive Mobile Fit) ---
export function loadNavbar() {
    const nav = document.getElementById('navbar');
    
    // 1. Render Structure
    nav.innerHTML = `
        <nav class="w-full bg-black/95 backdrop-blur-md fixed top-0 z-50 border-b border-gray-800 shadow-md h-[65px] md:h-[80px] flex items-center">
            
            <div id="nav-main" class="w-full max-w-screen-xl mx-auto px-2 md:px-4 flex items-center justify-between h-full">
                
                <a href="index.html" class="flex-shrink-0 w-[100px] md:w-[150px] h-full flex items-center overflow-hidden">
                ...
                <div class="flex items-center gap-1 md:gap-4 flex-shrink-0">
                    ...
                    <button onclick="window.toggleSearch()" class="text-white p-2 rounded-full focus:outline-none cursor-pointer">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    </button>

                    <a href="cart.html" class="relative text-white p-2 rounded focus:outline-none">
                    ...
                    <button id="menu-toggle" class="text-white p-2 rounded focus:outline-none">
                        <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16m-7 6h7"></path></svg>
                    </button>
                </div>
            </div>

            <div id="search-overlay" class="hidden fixed top-0 left-0 w-full h-[65px] md:h-[80px] bg-black z-[100] flex items-center px-4 border-b border-gray-800 shadow-2xl">
                <div class="w-full max-w-screen-xl mx-auto flex items-center gap-3">
                    ...
                    <button onclick="window.toggleSearch()" class="text-gray-400 font-bold px-3 py-2 text-sm uppercase tracking-wide cursor-pointer">Cancel</button>
                </div>
            </div>

            <div id="mobile-menu" class="hidden bg-[#111] border-b border-gray-800 absolute w-full left-0 top-[65px] md:top-[80px] shadow-xl z-40 h-screen">
                <ul class="flex flex-col font-medium text-lg" id="menu-list">
                    </ul>
            </div>
        </nav>
        
        <div class="h-[65px] md:h-[80px]"></div> 
